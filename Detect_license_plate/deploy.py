...
def deploy(img_path=None, vid_path=None,vid_out = None):

    print(f"[INFO] Loading model... ")
    ## loading the custom trained model
    # model =  torch.hub.load('ultralytics/yolov5', 'custom', path='yolov5s.pt',force_reload=True) ## if you want to download the git repo and then run the detection
    model =  torch.hub.load('Detect_license_plate\yolov5-master', 'custom', source ='local', path='Detect_license_plate\\best.pt',force_reload=True) ### The repo is stored locally
...
# deploy(img_path="Detect_license_plate\\test_images\\5.jpg") ## for image
